...
=============================================================
  ...
  MONACO'S APPROACH (from paper):
    ...
    Classifier: kNN with LOOCV (k = sqrt(N-1), Monaco's own rule)
...
# ══════════════════════════════════════════════════════════
def knn_classify(labeled_data, feat_key, scales, k=None):
    if k is None:
        import math as _math
        k = round(_math.sqrt(len(labeled_data) - 1))
...
def analyse_single_node(node, timestamps_out, timestamps_in,
                        ...
                        timing_scales, network_scales, k=None):
    import math as _math
    if k is None:
        k = round(_math.sqrt(len(labeled_data) - 1))
    THIN2 = "-" * 65
    ...
    if mf:
        ...
        out_count = len(timestamps_out.get(node, []))
        print()
        print("  MONACO FEATURES: NOT AVAILABLE")
        print("  Reason: Monaco needs at least 2 outgoing ratings to")
        print("  compute RTI (time between transactions).")
        print("  Node %d has only %d outgoing rating(s)." % (node, out_count))
        print()
        print("  This is Monaco's weakness -- nodes with few transactions")
        print("  cannot be classified at all.")
        print("  Our classifier works even with 1 incoming rating")
...
def main():
    ...
    print()
    print("  WHY %d and not %d (all nodes)?" % (len(labeled_data), len(all_nodes)))
    print("  Monaco REQUIRES 2+ outgoing ratings to compute RTI.")
    print("  So for a FAIR comparison we test Monaco only on nodes")
    print("  where it CAN work. This gives Monaco its best chance.")
    print()
    print("  Our classifier coverage: %d/%d nodes (%.1f%%)" % (
        sum(1 for n in all_nodes if incoming_norm.get(n)),
        len(all_nodes),
        sum(1 for n in all_nodes if incoming_norm.get(n))/len(all_nodes)*100))
    print("  Monaco coverage        : %d/%d nodes (%.1f%%)" % (
        sum(1 for n in all_nodes if len(timestamps_out.get(n,[]))>=2),
        len(all_nodes),
        sum(1 for n in all_nodes if len(timestamps_out.get(n,[]))>=2)/len(all_nodes)*100))
    print("  (Monaco cannot classify nodes with <2 outgoing ratings)")
    ...
    # ── CORRECT k (Monaco Section 5 rule: k = sqrt(N-1)) ────
    # Monaco's dataset: 366 samples → k = sqrt(365) = 19.1 ≈ 20
    # Our dataset: N labeled nodes  → k = sqrt(N-1)
    import math as _math
    N = len(labeled_data)
    k = round(_math.sqrt(N - 1))

    # Run all classifiers
    print("\n[3] RUNNING CLASSIFIERS")
    print("  Monaco's k rule: k = sqrt(reference_set_size)")
    print("  Monaco: 366 samples → k = sqrt(365) = 19.1 ≈ 20")
    print("  Ours  : %d samples → k = sqrt(%d)  = %.1f ≈ %d  (correct for our dataset)" % (
          N, N-1, _math.sqrt(N-1), k))
    print("  This may take a moment...", end="", flush=True)

    tp1,fp1,tn1,fn1 = knn_classify(labeled_data,'monaco',   timing_scales,   k)
    print(".", end="", flush=True)
    tp2,fp2,tn2,fn2 = knn_classify(labeled_data,'network',  network_scales,  k)
    print(".", end="", flush=True)
    tp3,fp3,tn3,fn3 = knn_classify(labeled_data,'combined', combined_scales, k)
    print(" done.\n")
